...
import { ArrowRight, ClipboardCheck, PanelsTopLeft, ShoppingBag, ShieldCheck } from 'lucide-react-native';
...
export const MyProjectScreen = ({ navigation }: any) => {
  ...
  useEffect(() => {
    if (isFocused && userId) void activeJourney.refetch();
  }, [activeJourney.refetch, isFocused, userId]);

  useEffect(() => {
    if (isFocused) void hydrateMaintenanceBookings();
  }, [hydrateMaintenanceBookings, isFocused]);

  if (activeJourney.isLoading) {
    return (
  ...
  if (latestMaintenanceBooking) {
    return (
      <Screen>
        <Header title={t('project.title')} subtitle="Your active service request" />
