...
export const MySolarJourneyScreen = ({ navigation, route }: any) => {
  ...
  const handleConfirmCancellation = async () => {
    ...
    setCancelError(null);
    if (!booking?.id) {
      setCancelError('Survey booking information is unavailable. Please refresh and try again.');
      return;
    }
    if (!userId) {
    ...
    if (!canCancelBooking) {
      setCancelError('This survey booking can no longer be cancelled because the survey process has already started.');
      return;
    }
    if (!selectedReason) {
    ...
    try {
      ...
      const result = await journeyApi.cancelSurveyBooking({
        ...
        cancellationNote: selectedReason === 'Other' ? trimmedOtherReason : null
      });
      ...
      if (!result.success || !result.booking) {
        throw new Error(result.message || 'Cancellation update returned no booking.');
      }

      const cancelledBooking = result.booking;
      queryClient.setQueryData(['survey-bookings', 'detail', cancelledBooking.id], cancelledBooking);
      queryClient.setQueryData(activeSurveyJourneyQueryKey(cancelledBooking.user_id), null);
      queryClient.setQueryData(latestSurveyJourneyQueryKey(cancelledBooking.user_id), cancelledBooking);
      await Promise.all([
        query.refetch(),
        queryClient.invalidateQueries({ queryKey: activeSurveyJourneyQueryKey(cancelledBooking.user_id) }),
        queryClient.invalidateQueries({ queryKey: latestSurveyJourneyQueryKey(cancelledBooking.user_id) }),
        queryClient.invalidateQueries({ queryKey: notificationsQueryKey(cancelledBooking.user_id) }),
        queryClient.invalidateQueries({ queryKey: unreadNotificationsQueryKey(cancelledBooking.user_id) }),
        queryClient.invalidateQueries({ queryKey: latestWelcomeNotificationQueryKey(cancelledBooking.user_id) }),
      ]);
      ...
      try {
        await notificationsApi.createSurveyCancellationNotificationOnce(cancelledBooking);
        await Promise.all([
          queryClient.invalidateQueries({ queryKey: notificationsQueryKey(cancelledBooking.user_id) }),
          queryClient.invalidateQueries({ queryKey: unreadNotificationsQueryKey(cancelledBooking.user_id) }),
        ]);
        ...
        if (__DEV__) {
          ...
          console.log('Survey cancellation notification creation failed', { message });
        }
      }
    } catch (error) {
      const errorLike = error as { code?: string; message?: string };
      const message = error instanceof Error ? error.message : errorLike?.message ?? 'Unknown cancellation error';
      ...
      if (__DEV__ && errorLike?.code === 'PGRST204') {
        setCancelError('Database migration is missing on the connected Supabase project. Apply the survey booking cancellation columns migration and reload the Supabase schema cache.');
      } else if (message.includes('No survey booking was updated')) {
        setCancelError('This survey booking could not be updated. Please refresh the page and try again.');
      } else {
        setCancelError('We could not cancel your survey booking. Please try again or contact our representative.');
      }
    } finally {
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {isCancelled ? (
          <View style={styles.cancelledCard}>
            ...
            <Text style={styles.cancelledText}>Reason: {booking.cancellation_reason || 'Not provided'}</Text>
            {booking.cancellation_note ? <Text style={styles.cancelledText}>Note: {booking.cancellation_note}</Text> : null}
            <Text style={styles.cancelledText}>Cancelled on: {formatDate(booking.cancelled_at)}</Text>
            <Pressable style={styles.newSurveyButton} onPress={() => navigation.navigate('BookSurvey')}>
        ...
        {canCancelBooking ? (
          <Pressable style={styles.cancelButton} onPress={openCancelModal} accessibilityRole="button">
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.keyboardAvoider}>
            <View style={styles.cancelSheet}>
                ...
                <View style={styles.reasonList}>
                  {cancellationReasons.map((reason) => {
                    ...
                    return (
                      <Pressable
                        key={reason}
                        style={[styles.reasonOption, selected && styles.reasonOptionSelected, isCancelling && styles.reasonOptionDisabled]}
                        onPress={() => {
              ...
              />
                ...
                {cancelError !== null ? <Text style={styles.cancelErrorText}>{cancelError}</Text> : null}
              </ScrollView>

              <View style={styles.cancelActions}>
                <Pressable style={[styles.keepButton, isCancelling && styles.actionDisabled]} onPress={closeCancelModal} disabled={isCancelling}>
                  <Text style={styles.keepButtonText}>Keep My Booking</Text>
                </Pressable>
                <Pressable style={[styles.confirmCancelButton, isCancelling && styles.confirmCancelButtonDisabled]} onPress={handleConfirmCancellation} disabled={isCancelling}>
                  {isCancelling ? <ActivityIndicator color="#FFFFFF" size="small" /> : null}
                  <Text style={styles.confirmCancelText}>{isCancelling ? 'Cancelling...' : 'Yes, Cancel Survey'}</Text>
                </Pressable>
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  cancelMessage: { marginTop: 8, color: '#64748B', fontSize: 13, fontWeight: '600', lineHeight: 20 },
  reasonLabel: { marginTop: 16, color: '#10213A', fontSize: 12, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 0.4 },
  reasonList: { marginTop: 8, gap: 8 },
  reasonOption: { minHeight: 42, flexDirection: 'row', alignItems: 'center', gap: 10, borderRadius: 13, borderWidth: 1, borderColor: '#E7DFD1', backgroundColor: '#FFFFFF', paddingHorizontal: 12 },
  reasonOptionSelected: { borderColor: '#D14343', backgroundColor: '#FFF2F2' },
  reasonOptionDisabled: { opacity: 0.68 },
  reasonRadio: { width: 14, height: 14, borderRadius: 999, borderWidth: 2, borderColor: '#B7BFC9' },
  ...
  reasonInput: { marginTop: 10, minHeight: 82, borderRadius: 13, borderWidth: 1, borderColor: '#F0B4B4', backgroundColor: '#FFFFFF', paddingHorizontal: 12, paddingVertical: 10, color: '#10213A', fontSize: 13, fontWeight: '600', textAlignVertical: 'top' },
  cancelErrorText: { marginTop: 10, color: '#B42318', fontSize: 12, fontWeight: '800', lineHeight: 18 },
  cancelActions: { marginTop: 16, flexDirection: 'row', gap: 10 },
  keepButton: { flex: 1, minHeight: 48, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: '#F3F0EA' },
  actionDisabled: { opacity: 0.62 },
  keepButtonText: { color: '#10213A', fontSize: 12.5, fontWeight: '900' },
  confirmCancelButton: { flex: 1, minHeight: 48, flexDirection: 'row', gap: 8, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: '#D14343' },
